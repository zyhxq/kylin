...
public class BlockingReservoir extends AbstractActiveReservoir {
    ...
    public BlockingReservoir(int minReportSize, int maxReportSize, int MAX_REPORT_TIME) {
        ...
        scheduledReporter = new ThreadFactoryBuilder().setNameFormat("metrics-blocking-reservoir-scheduler-%d").build().newThread(new ReporterRunnable());
    }
    ...
    private void onRecordUpdate(boolean ifAll) {
        ...
        for (ActiveReservoirListener listener : listeners) {
            if (!notifyListenerOfUpdatedRecord(listener, records)) {
                ifSucceed = false;
                logger.warn("It fails to notify listener " + listener.toString() + " of updated records " + records.toString());
            }
        }
        if (!ifSucceed) {
    ...
    private boolean notifyListenerHAOfUpdatedRecord(List<Record> records) {
        logger.info("The HA listener " + listenerHA.toString() + " for updated records " + records.toString() + " will be started");
            ...
            while (isReady) {
                if (size() <= 0) {
                    ...
                    logger.info("The number of records in the blocking queue is less than " + MIN_REPORT_SIZE + //
                            " and the duration from last reporting is less than " + MAX_REPORT_TIME + "ms. Will delay to report!");
                    sleep();
                    continue;
                }
